...
char CodMo(char *ss, char *sss) { //найти код микрооперации *sss в строке *ss
  char locs[5];
  char x;
  char *namemo = "      ";
  char *namemo1 = "        ";
...
exita:
  cod = 0; //ошибка символа в поле микрооперации
  return cod;
}
...
void TakeCode1(char *stpole, char *stmo)  { //сформировать коды в полях *stpole  для микрооперации *stmo
...
void TakeCode4(char *stpole, char *stmo) { //сформировать коды в полях *stpole  для микрооперации *stmo
...
void TakeCode2(char *stpole, char *stmo) //побитовое формирование поля унитарных сигналов
{
...
void TakeCode7(char *stpole, char *stmo) { //сформировать коды в полях *stpole  для микрооперации *stmo
...
//блок7   Control  (dcm8.mif )  + interrupt
//========================================================
//сокращенные адреса рабочих регистров при обращении к теневым в SFR
uchar unicontr;
...
void Takeromm(
    char *stpole,
    char *stmo) { //сформировать коды в полях *stpole  для микрооперации *stmo
...
void __fastcall TForm1::TakeCode(
    char *stpole,
    char *stmo) { //формирование кодов в полях *stpole  для микрооперации *stmo
  priznak = 0;
...
{ //*ss -символьная строка микрокоманды
  if (CheckBox1->State == cbChecked) //разрешение кодирования
  {
      ...
      for (char ii = 0;; ii++) {
        ...
        if (xx == '=') //правая часть -имя микрооперации
          if ((x == ',') || (x == ' ')) //конец микрооперации
          {
            ...
            xx = x;
            if (x == ' ')
              goto finn; //конец микрокоманды
          } else {
            *namemo++ = x;
            *namemo = 0;
          } //выборка имени микрооперации
        else // xx=',' выборка имени поля микрооперации
            if (x == '=')
          xx = '='; //переключение на выборку имени микрооперации
        else {
          *namepole++ = x;
          *namepole = 0;
        }
    ...
    finn:
      CompareCode(); //завершение разбора структурной
...
void __fastcall TForm1::GoToInt(void) //схема входа в прерывание //+ 0-микрококоманда
{
  ...
  if (!intra2) {
    ...
    intra = intr0 | intr1;
    if (intra)
      intra2 = 1; //аппаратный признак входа в прерывание
    if (intr0) {
      vect = 0x3;
      intr0 = 0;
      IE0 = 0;
...
void __fastcall TForm1::PSWC(char *simv) //формирование слова состояния
{
  ...
  if (S == Q) {
    AB = PA - PB - (PSW >> 7);
    PSW = (AB & 0x100) ? PSW & 0x7f : PSW | 0x80; //заем=неC
    PSW = ((~(PA ^ (~PB)) & 0x80) & (PA ^ AB)) ? PSW | 0x04 : PSW & 0xfb; // OV
  }
...
void __fastcall TForm1::StateMCU(void) { //текущее состояние памяти
  ...
  Edit1->Text = &Q[0]; //вывод битовой строки PSW
  bitreg(P3);
  Edit2->Text = &Q[0];
...
uchar vector[0x10];
void __fastcall TForm1::Button1Click(TObject *Sender) { //функция клавиши  Сброс
  uchar i;
  int k;
  // массив векторов ==========================
  ...
  ADC[0] = 0; //команда NOP-->0

  ADC[0x02] = 1; // команда ljamp ad16
  for (i = 0x08; i <= 0x0f; i++)
    ADC[i] = 2;  // inc ri
  ADC[0x05] = 3; // inc ad
  ADC[0xa3] = 4; // inc dptr
...
void __fastcall TForm1::Button2Click(TObject *Sender) {
  ...
  case 2: // inc ri
  {
    ...
    itoa(IR & 0x07, &ss[0], 16); //формирование мнемокода
    char stroka[] = "inc r";
    Instr->Text = StrCat(stroka, ss);
    Edit7->Text = Ram[addr];
  }
  ...
  case 3: // inc ad
  {
  ...
  }
    {
      ...
      Ram[PB] = Wrk + 1;
      RAMK++;
      char tt[] = "inc  "; //формирование мнемокода
  ...
  case 6: // xchd a,@ri
  {
    // 11.1 -чтение операндов из АСС и памяти Code
    PB = Ram[IR & 1]; // CODE[PC++];
    RAMK++;
    ...
    // 11.2 - - операция в АЛУ и формирование признаков результата
    {
      Wrk = ACC;
      RAMK++;
    ...
    goto finish;
  case 7: //  lcall  met // исправить LCALL ad16
  {

    // 9.1 - чтение второго байта команды и преинкремент указателя
    Wrk = CODE[PC++];
    SP++;
    RAMK++;
    ...
    {
      // 9.2- запись в Стек PC(7-0) и постинкремент указателя
      Ram[SP++] = PC;
      RAMK++;
    ...
    {
      // 9.3      PC(15-8) --> Стек,формирование новогоPC
      Ram[SP] = PC >> 8;
      RAMK++;
      PC = (Wrk << 8) + (CODE[--PC]);
    ...
    {
      // 9.4 сохранение продвинутого указателя в SFR и завершение микропрограммы
      Ram[Sp] = SP;
      RAMK = 0;
      MicroCodMem(
...
char Chi;
void __fastcall TForm1::CheckBox1Click(TObject *Sender) { //"кодирование разрешить"
  if (Chi == 0) {
...
void ToFile(AnsiString S, char *ss) { //создать  .mfi файл для maxplus
  char *std = "     ";
  char *std1 = "     ";
  char *std2 = "     ";
  char *std3 = "     ";
...
void __fastcall TForm1::Button7Click(TObject *Sender) { //сохранить все в .mfi файлы
...
void __fastcall TForm1::Button3Click(TObject *Sender) { //запрос прерывания по H/L фронтуINT1 при IT1=1
  IE1 = 1;
  TCON = TCON | 0x8;
  Ram[Tcon] = TCON;
  CheckBox6->State = cbChecked;
...
void __fastcall TForm1::CheckBox2Click(TObject *Sender) { //внешняя установка маски INT1
...
void __fastcall TForm1::CheckBox3Click(TObject *Sender) { //внешняя установка маски INT0
...
void __fastcall TForm1::CheckBox4Click(TObject *Sender) { //внешнее разрешение прерываний
...
void __fastcall TForm1::Edit7DblClick(TObject *Sender) { //дваждыжмяк по текстовому полю где RAM
  char s;
  char ss[3];
  int L;
...
void __fastcall TForm1::Edit6DblClick(TObject *Sender) { //где имя SFR
  uint numb = 0xffff;
...
void __fastcall TForm1::Edit9DblClick(TObject *Sender) { //текстовое поле xdata
  char s;
  char ss[3];
  int L;
...
void __fastcall TForm1::CheckBox6Click(TObject *Sender) { //галка запроса IE1
  /* IE0^=1;
    if (~IE0)
...
void __fastcall TForm1::Button13Click(TObject *Sender) { //кнопка читать файл Code
  //я не хочу реверсить эту дичь, я умственно нормальный
  AnsiString S;
  char j = 0;
  char numb = 0;
  int adr = 0;
...
void __fastcall TForm1::Edit2DblClick(TObject *Sender) { //текстовое поле P3
  char i;
  int ss;
  char qq[18];
  AnsiString S;
